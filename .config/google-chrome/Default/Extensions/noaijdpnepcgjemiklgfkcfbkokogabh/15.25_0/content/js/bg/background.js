ImTranslatorBG = {
	...
	init: function(){
		...
		if (ImTranslatorBG.first_run) { 
			...
			ImTranslatorBG.SL_RunWelcomePage(); 
                        ...
                        if(FExtension.store.get("SL_Version") != version){
                        	if(FExtension.store.get("SL_Version") != "15.22"){
					FExtension.store.set("SL_Version", version);
					ImTranslatorBG.SL_RunWelcomePage(); 
				}else{
					FExtension.store.set("SL_Version", version);
		...
		if(FExtension.store.get("SL_CM5")==1){
			ImTranslatorBG.the_ID5 = FExtension.browser.createContextMenus(FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extCMMouseOverTransTo') + " " +ImTranslatorBG.Lexicon(ImTranslatorBG.GetLongLanguageName(FExtension.store.get("SL_langDst_wpt"))),ImTranslatorBG.SL_WEB_PAGE_TRANSLATION_MO, false);
		}
...
//	 if(URL_host.indexOf("https://")>-1) {alert(FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extWPTalert1'));GOhead=1;}
	 ...
	 if(GOhead==0) ImTranslatorBG.SL_WEB_PAGE_TRANSLATION_MO();
...
*/
		...
		if(GOhead==0){
			ImTranslatorBG.THE_URL = "https://translate.googleusercontent.com/translate_c?depth=1&anno=2&hl="+chrome.i18n.getUILanguage()+"&rurl=translate.google.com&sl="+langS+"&tl="+langD+"&u="+URL_host;
	...
	SL_RunWelcomePage: function(){
	 ...
	 switch(ImTranslatorBG.ADVkey){
	  case 1: FExtension.browser.openNewTab("http://imtranslator.net/Translator-for-"+PLATFORM+"-Imtranslator.asp"); break;
	  case 2: if(ImTranslatorBG.first_run==true) FExtension.browser.openNewTab("http://imtranslator.net/Translator-for-"+PLATFORM+"-Imtranslator.asp"); break;
	  case 3: if(ImTranslatorBG.first_run==false) FExtension.browser.openNewTab("http://imtranslator.net/Translator-for-"+PLATFORM+"-Imtranslator.asp"); break;
	 }
	},
