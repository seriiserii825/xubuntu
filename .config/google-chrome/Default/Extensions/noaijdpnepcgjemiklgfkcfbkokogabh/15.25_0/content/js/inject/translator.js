...
TranslatorIM = {
	...
	MOver_PROVIDERS:function(e){
	    try{
	        var id = e.target.id.replace("SL_P","");
		if(e.target.id.indexOf("SL_P")!=-1){
	...
	keyup:function(e){ 
		...
		if(TranslatorIM.SL_MSG.indexOf("~")!=-1){
			...
			if(TranslatorIM.SL_WPTbox2 == "true"){
	                        if(TranslatorIM.SL_SYNC_FILTER(DBHOTKEYSlineWPT2,HOTKEYSline)==true) TranslatorIM.SL_WEB_PAGE_TRANSLATION_MO(1);
	                }
			//OPT
...
*/
	...
	SL_GetButtonCurPosition: function (X1,Y1,X2,Y2) {
	        ...
	        var SLdivField = doc.getElementById("SL_button");
                ...
                var correctionX = TranslatorIM.GlobalCorrectionX;
                var correctionY = TranslatorIM.GlobalCorrectionY;
...
*/
		...
		var SL_P4 = doc.getElementById("SL_P4");
	...
	SL_bbl_help: function(){
		switch(PLATFORM){
			 case "Opera" : var slurl="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-opera/opera-bubble-translator/"; break;
			 case "Chrome": var slurl="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-chrome/chrome-bubble-translator/"; break;
			 default      : var slurl="http://about.imtranslator.net/tutorials/presentations/";break;
		}
	  ...
	  if(e){
		...
		if(cl!="SL_options" && (id.indexOf("SL_")==-1 || id=="SL_button")){
			...
			if(id!="" || cl!=""){
				...
				if(id=="SLO_arrow_down" || id=="SLO_arrow_up" || id=="SLO_Balloon_Close" || cl=="SLO_options"){ 
					var sel = window.getSelection ? window.getSelection() : document.selection;
					SLdivField.style.backgroundColor = "#FEFEFE";
					TranslatorIM.SLO_ShowBalloon();
					sel.removeAllRanges();
				}
...
//	  if(BackUpDir=="es") client="t";
	...
	SL_CopyToClipBoard: function(){
		...
		setTimeout(function(){ OBJ.style.opacity=1; }, 500);
...
/////////
	...
	SL_EXECUTE_TRANSLATION: function(myTransText,evt,st, win) {
           ...
           myTransText = TranslatorIM.SL_TEMP_TEXT;
//	   myTransText = FExtension.browserInject.getSelectionText();
...
//	        if(S=="auto" && TranslatorIM.SL_DETECT != "") S=TranslatorIM.SL_DETECT;
		...
		if(text != ""){
		  ...
		  if(S==T){
			...
			doc2.getElementById('SL_shadow_translation_result').innerHTML=resptmp.replace(/@/ig,'<br>');
			doc2.getElementById('SL_shadow_translation_result2').innerHTML=resptmp.replace(/@/ig,'<br>');
			...
			if(PR.toLowerCase()=="google")  text=text.replace(/@/ig,'^');
...
//					        			OB23.setAttributeNode(js);
						...
						if(TranslatorIM.SL_SHOW_PROVIDERS!="1"){
							...
							ti.value = TranslatorIM.LISTofPR[p];
						        ...
						        OBprov.setAttributeNode(id);
                                                        OBprov.appendChild(doc.createTextNode(TranslatorIM.LISTofPR[p][0]));
		...
		OBtbl.height = "16";
				...
				OBtd2.align = "center";
					...
					hr = doc.createAttribute("href");
					hr.value ="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=GD9D8CPW8HFA2";
...
//			}
	...
	SL_WEB_PAGE_TRANSLATION_MO: function(st){
		var URL_host= String(window.location);
		var langS=TranslatorIM.SL_WPTsrclang;
		var langD=TranslatorIM.SL_WPTdstlang;
		var GOhead=0;
		if(URL_host.indexOf("file:///")>-1) {
			alert(FExtension.element(TranslatorIM.SL_LOC,"extWPTalert2"));GOhead=1;
		}
		if(GOhead==0){
			var SLnow = new Date();
			SLnow=SLnow.toString();
            		...
            		var CurDT=TMPtime[1]+" "+TMPtime[2]+" "+TMPtime[3]+", "+TMPtime[4];
            		var tp=3;
			var THE_URL = "https://translate.googleusercontent.com/translate_c?depth=1&anno=2&hl="+chrome.i18n.getUILanguage()+"&rurl=translate.google.com&sl="+langS+"&tl="+langD+"&u="+URL_host;
			FExtension.browserInject.runtimeSendMessage({greeting: URL_host + "~~" + URL_host + "~~" + langS + "|" + langD + "~~"+ URL_host+"~~"+CurDT+"~~"+tp+"^^"}, function(response) { });
...
*/
	...
	//         '    pageLanguage: "' + sourceLang + '", ',
	      ...
	      translateElementScript.src = '//translate.google.com/translate_a/element.js?cb=googleTrButton' + uid + '&hl=' + sourceLang;
...
//        },1200);  
    ...
    SL_RemoveExistingGoogleTranslateWidget: function() {
      var ObFound = false;
      var TrScrs = document.querySelectorAll('script[src*="//translate.google.com/"]');
	 ...
	 SL_WPT_TABS.setAttributeNode(id);
	  ...
	  SL_WPT_TAB2div.onclick = function(){
		...
		SL_WPT_TAB5div.onclick = function(){
		    window.open("http://about.imtranslator.net/tutorials/presentations/imtranslator-for-chrome/chrome-webpage-translation/","ImTranslator:Help");
		};
		var id = doc.createAttribute("id");
		...
		SL_WPT_TAB6div.onclick = function(){
		    window.open("https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=GD9D8CPW8HFA2","ImTranslator:Donate");
		};
		var id = doc.createAttribute("id");
...
//			 	case "en":LNG = "en-GB|Male"; break;
	...
	PPB_tts_icon: function (T,resp){
		...
		if(TranslatorIM.SL_ALLvoices.indexOf(T)!=-1){
			...
			else 	var tmpTTSicn='<div class="PPB_voice1" title="'+FExtension.element(TranslatorIM.SL_LOC,"extListen_ttl")+'" id="SL_BBL_VOICE" style="background: url('+FExtension.browserInject.getURL("content/img/util/tts.png")+');" lang="'+doc2.getElementById("SL_lng_to").value+'">&nbsp;</div>';
			resptmp = tmpTTSicn+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+resptmp;
		}
		return resptmp;
	},
	...
	SL_YANDEX: function(text,f,t){
		...
		if(f==t){
			...
			doc.getElementById('SL_shadow_translation_result').innerHTML=resptmp.replace(/@/ig,'<br>');
			doc.getElementById('SL_shadow_translation_result2').innerHTML=resptmp.replace(/@/ig,'<br>');
			...
			setTimeout(function(){
			    var SLIDL = setInterval(function(){
					...
					if(TranslatorIM.BBL_RESULT!="") {
						...
						text=text.replace(/#/ig,"'");
					    	if(text!="" && text!="<'>"){
							TranslatorIM.SL_temp_result = text.replace(/@/ig,'');
							text=text.replace(/@/ig,'<br>');
	       					        var resptmp = TranslatorIM.PPB_tts_icon(t,text);
			        			doc.getElementById('SL_shadow_translation_result').innerHTML=resptmp;
							doc.getElementById('SL_shadow_translation_result2').innerHTML=resptmp;
			                		doc.getElementById('SL_shadow_translation_result').style.direction = "ltr";
							doc.getElementById('SL_shadow_translation_result').style.textAlign = "left";
