...
//	if(st==0) var cgi = "dir="+DetLang+"/"+ln+"&provider=google&text="+encodeURIComponent(text);
...
function injectScriptRear(id, dictionary, text, langDst) {
    var xhr = getHttpRequest();
    var url = ImTranslator_theurl+"dotrans_20160909.php";
...
function SaveTransToHistory(text,historyText) {
	    ...
	    if(TranslatorIM.INLINEflip==0) mySourceLang = "auto";
            ...
            FExtension.browserInject.runtimeSendMessage({greeting: "[i]" + text + "~~" + historyText + "~~" + mySourceLang + "|" + myTargetLang + "~~" + FExtension.browserInject.getDocument().location + "~~" + CurDT + "~~5^^"}, function (response) {
                if(response){
