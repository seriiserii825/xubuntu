...
(function(){document.addEventListener("mousedown",function(){
   ...
   if(GEBI("SL_myRange")){
	if(SL_getCookie("TTSvolume")==null || SL_getCookie("TTSvolume")=="undefined" || SL_getCookie("TTSvolume")=="") SL_setCookie("TTSvolume","5");
	else SL_setCookie("TTSvolume",GEBI("SL_myRange").value);
   ...
   if(id == "SL_volume"){
	...
	SL_setCookie("TTSvolume",GEBI("SL_myRange").value);
...
function SL_Tabs_Settler(){
 ...
 if(GEBI(id).className!="SL_LABLE_DEACT"){
	 SL_setCookie("DPROV",LISTofPR[ind]);
//	 var t=SL_DICT();
	 REMOTE_Voice_Close();
 }
 SET_PROV(ind);
...
//	GEBI('SL_DICTsource').appendChild(document.createTextNode(FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extDictionary')));
	...
	switch(PLATFORM){
	 case "Opera" : GEBI('SLhelp_a').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-opera/opera-imtranslator-dictionary/"; break;
	 case "Chrome": GEBI('SLhelp_a').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-chrome/google-dictionary/"; break;
	 default      : GEBI('SLhelp_a').href="http://about.imtranslator.net/tutorials/presentations/";break;
	}
...
//        FExtension.store.set("CUR_URL","undefined");
  ...
  ajaxRequest.onreadystatechange = function(){
		if(ajaxRequest.readyState == 4){
			...
			if(localStorage["SL_TS"]!=SL_getCookie("OLD_TS")){
			   	localStorage["SL_Flag"]="FALSE";
			   	SL_setCookie("OLD_TS",localStorage["SL_TS"]);
				SL_setCookie("DPROV","");
				PROV="";
				SL_setCookie("DIC_FIRSTRUN","");
//	SET_PROV();
//        SET_FIRST_AVAILABLE_PROV();
...
function SET_DICT_PROVIDER(){
  for(var I=0; I<LISTofPR.length; I++){
    if(SL_getCookie("DPROV") == LISTofPR[I]) GEBI("SL_P"+I).className="SL_TAB_DICT";
    else GEBI("SL_P"+I).className="SL_TAB_OFF_DICT";   
  }
}

function SL_SET_DICT_PRIVIDER(pr){
	SL_setCookie("DPROV",pr);
	SET_PROV();
	SET_FIRST_AVAILABLE_PROV();
	SL_INIT_DICT();
}
...
function SET_PROV(){
  ListProviders="";
//  if(SL_getCookie("DPROV") == "" || SL_getCookie("DPROV") =="undefined"){ SL_setCookie("DPROV","Google"); }
  for(var I=0; I<LISTofPR.length; I++){
...
//         f=GEBI('SL_langSrc').value;
//	 t=GEBI('SL_langDst').value;
...
function SET_FIRST_AVAILABLE_PROV(){
 if(SL_getCookie("DIC_FIRSTRUN")!="dicdone"){
  	...
  	if(localStorage["SL_dict"]=="true"){
	  	...
	  	for(var I=0; I<(theList.length-1); I++){
				...
				if(arr2[1]==1 && theList[I]==arr2[0]){
					SL_setCookie("DPROV",arr2[0]);
			      		PROV=arr2[0];
			      		GEBI("SL_P"+J).className="SL_TAB_DICT";
					SET_DICT_PROVIDER();
					SL_setCookie("DIC_FIRSTRUN","dicdone");
					I=1000;J=1000;
				}
		    	}
		}
  	} else {
		if(ListProviders.indexOf(SL_getCookie("DPROV"))==-1){
		      var theList = ListProviders.split(",");
		      for(var I=0; I<(theList.length-1); I++){
				PROV=theList[I];
			        SL_setCookie("DPROV",PROV);
			...
			if(SL_getCookie("DPROV")!=""){
	   			var theList = ListProviders.split(",");
   				for(var I=0; I<(theList.length-1); I++){
					if(theList[I] == SL_getCookie("DPROV")){
	      					GEBI("SL_P"+I).className="SL_TAB_DICT";
      						break;
					}
		   		}
				...
				var arr = localStorage["SL_ALL_PROVIDERS_GT"].split(",");
			        SL_setCookie("DPROV",theList[0]);
			        for(var J=0; J<arr.length; J++){
	   				for(var I=0; I<(theList.length-1); I++){
						if(theList[I] == arr[J] && theList[I] == SL_getCookie("DPROV")){
	      						GEBI("SL_P"+J).className="SL_TAB_DICT";
      							I=1000;J=1000;
							SL_setCookie("DIC_FIRSTRUN","dicdone");
						}
...
//			 	case "en":LNG = "en-GB|Male"; break;
			...
			if(SL_getCookie("TTSvolume")!=null && SL_getCookie("TTSvolume")!="undefined" && SL_getCookie("TTSvolume")!="") TheVolume = SL_getCookie("TTSvolume");
			...
			PLANSHET.innerHTML=PLAYER;
                        ...
                        //SL_setCookie("TTSvolume",TheVolume);
			...
			utterance.rate = SP;
			utterance.volume = SL_getCookie("TTSvolume")*1/10;
	...
	if(SL_getCookie("TTSvolume")>0)	GEBI("SL_volume").className="SL_volume";
	else 	GEBI("SL_volume").className="SL_novolume";

	GEBI("SL_myRange").value = SL_getCookie("TTSvolume");
