...
function SL_Tabs_Settler(){
 ...
 if(GEBI(id).className!="SL_LABLE_DEACT"){
	 SL_setCookie("PROV",LISTofPR[ind]);
	 var t=LETSTRANS();
	 if(t==true)Loader(1);
	 REMOTE_Voice_Close();
 }
...
function CONSTRUCTOR(){
	...
	GEBI('SL_h3').innerText="v."+FExtension.bg.ImTranslatorBG.Version();  
        if(SL_getCookie("PROV")=="" || SL_getCookie("PROV")=="undefined") PROV=LISTofPR[0];
        else PROV=SL_getCookie("PROV");
	...
	switch(PLATFORM){
	 case "Opera" : GEBI('SLhelp_a').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-opera/opera-imtranslator-tool/"; break;
	 case "Chrome": GEBI('SLhelp_a').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-chrome/imtranslator-tool/"; break;
...
function SET_FIRST_AVAILABLE_PROV(){
 if(localStorage["SL_other_gt"]=="1"){   
  if(SL_getCookie("TR_FIRSTRUN")!="done"){
...
function SET_PROV(st){
  ListProviders="";
  if(SL_getCookie("PROV") == "" || SL_getCookie("PROV") =="undefined"){ SL_setCookie("PROV","google"); }
  for(I=0; I<LISTofPR.length; I++){
    ...
    if(SL_getCookie("PROV") == LISTofPR[I]) GEBI("SL_P"+I).className="SL_LABLE";
    else                    GEBI("SL_P"+I).className="SL_LABLE_OFF";   
...
function SESSION(){        
	...
	ajaxRequest.onreadystatechange = function(){
		if(ajaxRequest.readyState == 4){
		    ...
		    if(localStorage["SL_TS"]!=SL_getCookie("OLD_TS_TR")){
			localStorage["SL_Flag"]="FALSE";
			SL_setCookie("OLD_TS_TR",localStorage["SL_TS"]);
        	        SL_setCookie("PROV","");
			SL_setCookie("TR_FIRSTRUN","");
                        SET_PROV(1);
		    }
		}
	}
...
function TranslateLOC(myTransText) {
        ...
        if(SL_getCookie("PROV")=="" || SL_getCookie("PROV")=="undefined") PROV="Google";
        else PROV=SL_getCookie("PROV");
...
function LOCcontrol(st){
    ...
    if(st==0) SL_setCookie("LOC",String(GEBI('SLlocpl').checked));
}
		...
		ajaxRequest.onreadystatechange = function(){
			if(ajaxRequest.readyState == 4){
			     ...
			     if(ind!=2){
				    ...
				    SL_alert(msg);
				    GEBI("SL_alert").style.height="165px";
			     	    GEBI('SL_indicator'+ind).style.display='none';
			    }
