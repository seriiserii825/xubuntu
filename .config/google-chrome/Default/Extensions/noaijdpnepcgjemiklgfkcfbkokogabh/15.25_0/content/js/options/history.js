...
setTimeout(function() {
 (function(){if(GEBI('SL_save_button'))GEBI('SL_save_button').addEventListener("click",function(){SL_SAVE();},!1);} )();
}, 1000);
...
//	  console.log(">>>>>>>>>>"+SL_getCookie("SL_anchor"))
	  if(SL_getCookie("SL_anchor")!="") document.location="history.html#"+SL_getCookie("SL_anchor");
	},2500);

	INIT();
	SL_record_finder(e);
	if(SL_getCookie("SL_sort")=="") SL_setCookie("SL_sort","0");
	if(SL_getCookie("SL_sort")!="0" || SL_getCookie("SL_sort")!="" || SL_getCookie("SL_sort")!="undefined" ) {
	 GEBI("SL_SORT").value=SL_getCookie("SL_sort");
	}
 	SL_SORT();
...
function CONSTRUCTOR(){
	...
	GEBI('SL_btn1').appendChild(document.createTextNode(FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extSaveButton')));
	GEBI('SL_il').appendChild(document.createTextNode(FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extLOC')));

	switch(PLATFORM){
	 case "Opera" : GEBI('SL_info').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-opera/opera-translation-history/"; break;
	 case "Chrome": GEBI('SL_info').href="http://about.imtranslator.net/tutorials/presentations/imtranslator-for-chrome/translation-history/"; break;
	 default      : GEBI('SL_info').href="http://about.imtranslator.net/tutorials/presentations/";break;
	}
}
...
function INIT(){  
 ...
 if(SL_getCookie("SL_sort")) GEBI("SL_SORT").value = SL_getCookie("SL_sort");
}
...
function SL_save_content_to_file(){
	...
	if(TMP[5]!='6'){
	        TMP_HISTORY_LIST = TMP_HISTORY_LIST + TMP[4] + "," + title + "," + TMP[3] + "," + DIR_from + ">" + DIR_to + " ("+GetLongProviderNmae(TMP[6])+")," + TMP_SRC + "," + TMP_DST + ",,,,\r\n"; 
	} else {
	   if(TMP_HIST[z].indexOf("<div")!=-1){
		...
		T1 = TMP_DST.split("<div id=_Y>");
		  ...
		  for(t=1; t<T1.length; t++){
	            	...
	            	if(t==1 && r==1){
			 TMP_HISTORY_LIST = TMP_HISTORY_LIST + TMP[4] + "," + title + "," + TMP[3] + "," + DIR_from + ">" + DIR_to + " ("+GetLongProviderNmae(TMP[6])+")," + TMP_SRC + "," + TR + "," + POS + "," + RT + "\r\n"; 
	            	}else{
		  ...
		  for(t=1; t<T0.length; t++){
			...
			TMP_HISTORY_LIST = TMP_HISTORY_LIST + TMP[4] + "," + title + "," + TMP[3] + "," + DIR_from + ">" + DIR_to + " ("+GetLongProviderNmae(TMP[6])+")," + TMP_SRC + "," + TR + "\r\n"; 
	            }
	          }
		}
	    } else {
...
function SL_SORT() {
 ...
 GEBI('SL_H_SEARCH').value="";
 SL_setCookie("SL_sort",GEBI("SL_SORT").value.toString());
// var mySL_HISTORY = FExtension.store.get("SL_History");
 var mySL_HISTORY = FExtension.store.get("SL_History").replace(/'dictionary.html/g,"'../../html/popup/dictionary.html");
 if(mySL_HISTORY!=""){
  var TMP_HIST=mySL_HISTORY.split("^^");
		...
		ajaxRequest.onreadystatechange = function(){
			if(ajaxRequest.readyState == 4){
			   ...
			   if(resp.indexOf('"trans":"')>-1){
			        ...
			        var GoogleResult=resp.split('"trans":"');
			        var GoogleResult2=GoogleResult[1].split('","');
				var GoogleResult3=GoogleResult2[0].replace(/<br>/ig,'@');
				GoogleResult3=GoogleResult3.replace(/&#39;/ig,"'");
				GoogleResult3=GoogleResult3.replace(/&quot;/ig,"'");
				GoogleResult3=GoogleResult3.replace(/&amp;/ig,"&");
				GoogleResult3=GoogleResult3.replace(/(<([^>]+)>)/ig,"");
				var ReadyToUseGoogleText=GoogleResult3.replace(/@/ig,"\n");
...
//  if(dir=="uk") client="t";
  ...
  if(!document.getElementById("PL_lbframe")){
    ...
    if(document.getElementById('SL_Hplayer'+CurTTSIND)) document.getElementById('SL_Hplayer'+CurTTSIND).appendChild(die);
     ...
     audioElement.setAttribute('style', 'width:710px;margin-top:-15px;margin-left:-20px;');
     ...
     document.getElementById('SL_Hplayer'+CurTTSIND).style.width="670px";
